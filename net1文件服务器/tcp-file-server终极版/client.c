...
int process_put(char *p, struct sockaddr_in serveraddr)//上传功能函数
{
	int sockfd;
	char buf[N] = {};
	int fd;
	...
	sprintf(buf, "P%s",p+4);//由函数传参把上传的文件名存入buf
	send(sockfd, buf, N, 0);//把文件名传给服务器

	if((fd = open(p+4, O_RDONLY)) < 0)//以只读的方式打开客户端要上传的文件
	{
		err_log("fail to open");
	}

	
	while((nbyte = read(fd, buf, N))> 0)//不断地读本地文件缓存到buf中
	{
		send(sockfd, buf, nbyte, 0);//不断地向服务器发送
		usleep(50);
	}
